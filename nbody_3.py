"""
    ...
    The runtime is 92.57 sec. 

    It is the forth best improvement. 
"""
...
def advance(dt):
    ...
    local = BODIES
    seenit = []
    for body1 in local.keys():
        for body2 in local.keys():
            if (body1 != body2) and not (body2 in seenit):
                ([x1, y1, z1], v1, m1) = local[body1]
                ([x2, y2, z2], v2, m2) = local[body2]
                (dx, dy, dz) = compute_deltas(x1, x2, y1, y2, z1, z2)
                update_vs(v1, v2, dt, dx, dy, dz, m1, m2)
                seenit.append(body1)
        
    for body in local.keys():
        (r, [vx, vy, vz], m) = local[body]
...
def nbody(loops, reference, iterations):
    ...
    # Set up global state
    local = BODIES
    offset_momentum(local[reference])
...
if __name__ == '__main__':
    ...
    t = Timer("nbody(100, 'sun', 20000)",'from __main__ import nbody')
